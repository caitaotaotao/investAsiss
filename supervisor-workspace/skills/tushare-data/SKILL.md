...
## Concurrency limit（并发限制 — 必须严格遵守）

**当前账户 Tushare API 最大并发数为 2。绝对不允许超过 2 个并发请求。**

这是硬性限制，违反会导致请求被拒绝或账号被限流。具体要求：

- 任何时刻，同时发出的 Tushare API 请求不得超过 **2** 个
- 批量拉取多标的或多时间段数据时，必须使用**串行**或**最多 2 并发**的方式执行
- 禁止使用 `asyncio.gather`、`ThreadPoolExecutor`、`multiprocessing` 等方式同时发起超过 2 个请求
- 如果使用并发工具，必须设置 `max_workers=2` 或 `semaphore=2`
- 每个请求之间建议间隔 **≥500ms**，避免瞬时并发撞限
- 分段拉取时，采用逐段串行方式，一个请求返回后再发下一个
...
for segment in segments:
    data = pro.daily(ts_code=ts_code, start_date=segment['start'], end_date=segment['end'])
    results.append(data)
...
with ThreadPoolExecutor(max_workers=2) as executor:
    futures = []
    for segment in segments:
        futures.append(executor.submit(fetch_segment, segment))
        time.sleep(0.5)
```
...
- **并发上限为 2，绝对不能超过**（详见 Concurrency limit 章节）
- 仅对瞬时错误（网络抖动、超时、429）进行有限重试
- 参数错误、权限不足、字段错误不要盲重试
- 批量拉取时加入节流，每次请求间隔 ≥500ms，避免高频撞限
- 多标的批量任务必须串行处理或最多 2 并发，不得使用更高并发度
...
- **并发请求不得超过 2 个**，这是硬限制，批量任务必须串行或限制 max_workers=2
- 先理解任务，再选接口
- 能少取就少取，先核心数据，再扩展
- 先给结论，再给证据
- 默认说人话，不堆字段名
